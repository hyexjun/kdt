import csv, requests, json, xmltodict
...
def pluspercent(ticker, toDate):
    ...
    covid["pluspercent"] = covid['dailydiff'].ge(
        changePerfloor) & covid['dailydiff'].lt(changePerceil)
    ...
    context = {'rate': rate, 'covidrate': covidrate, 'changePerfloor': changePerfloor, "changePerceil": changePerceil,
               'Date': Date, 'Change': Change, 'datetype': datetype, 'dailydecide': dailydecide}
    return context
...
def covidgoingon(date):  # 코로나 누적 확진자율, #코로나 증가 세기
    ...
    for i in range(1, 8):
        # 자동 변수 생성
        globals()['covid_'+str(i)] = covid['intdate'] == int(date) - \
            i  # 하루 전부터 7일 전까지
        globals()['covidCnt_'+str(i)] = int(covid[globals()
                                                  [f'covid_{i}']]['decideCnt'])  # 해당하는 날짜의 확진자 할당
...
def DailyCovid(request):
    ...
    covid['dailydecide'] = covid['decideCnt'].diff().fillna(
        0)  # covid 일일 신규 확진자 만들고
...
def DailyVaccine(request):
    ...
    vaccine.columns = ['strdate', 'intdate', 'firstCnt', 'secondCnt',
                       'thirdCnt', 'totalFirstCnt', 'totalSecondCnt', 'totalThirdCnt']
    ...
    context = {'intdate': intdate, 'firstCnt': firstCnt,
               'secondCnt': secondCnt, 'thirdCnt': thirdCnt}
    return JsonResponse(context)
...
def TotalVaccine(request):
    ...
    context = {'intdate': intdate, 'totalFirstCnt': totalFirstCnt,
               'totalSecondCnt': totalSecondCnt, 'totalThirdCnt': totalThirdCnt}
    return JsonResponse(context)
...
def code_DailyCovid(request):
    ...
    codedata['percent'] = codedata['Close'] / \
        codedata.iloc[0, 1]*100  # 2020-01-01의 종가를 기준으로 비율
    codedata.reset_index(inplace=True)  # indes 제거
    # DataFrame의 percent 열의 값들을 리스트로
    codeclose = codedata['percent'].values.tolist()
    covid = dailycovid.objects.all().values().order_by(
        'intdate')  # db에서 covid values 가져오기
    covid = pd.DataFrame(covid)  # 가져온 값들을 DataFrame
    covid.columns = ['strdate', 'intdate', 'deathCnt',
                     'decideCnt']  # 각각의 columns 에 이름 부여
    # DataFrame의 intdate 열의 값들을 리스트로
    intdate = covid['intdate'].values.tolist()
    covid['dailydecide'] = covid['decideCnt'].diff().fillna(0)
    dailydecide = covid['dailydecide'].values.tolist()
    ...
    context = {'codeclose': codeclose, 'intdate': intdate, 'dailydecide': dailydecide,
               'lowpoint': lowpoint, 'lowrate': lowrate, 'highpoint': highpoint, 'highrate': highrate}
    return JsonResponse(context)
...
def code_dailyVaccine(request):
    ...
    highpoint = highpointday(code)  # 언제 최고점?
    highrate = highration(code)  # 얼마나 상승 ?
    context = {'codeclose': codeclose, 'intdate': intdate, 'firstCnt': firstCnt, 'secondCnt': secondCnt,
               'thirdCnt': thirdCnt, 'lowpoint': lowpoint, 'lowrate': lowrate, 'highpoint': highpoint, 'highrate': highrate}
...
def code_totalVaccine(request):
    ...
    highpoint = highpointday(code)  # 언제 최고점?
    highrate = highration(code)  # 얼마나 상승 ?
    context = {'codeclose': codeclose, 'intdate': intdate, 'totalFirstCnt': totalFirstCnt, 'totalSecondCnt': totalSecondCnt,
               'totalThirdCnt': totalThirdCnt, 'lowpoint': lowpoint, 'lowrate': lowrate, 'highpoint': highpoint, 'highrate': highrate}
...
def covidData():
    ...
    # dic을 json 형식으로 만들었는데 이러면 json str타입이 나옴.
    ...
    for i in covidData:
        ...
        qs = dailycovid(strdate=date, intdate=int(date),
                        deathCnt=int(deathCnt), decideCnt=int(decideCnt))
        qs.save()
    # rowdata.append(date)
    ...
    for i in range(356):  # 페이지 별로
        ...
        for i in range(18):  # 날짜 row 별로
            if contents['data'][i]['sido'] == '전국':
                data.append(contents['data'][i]['baseDate'][0:4]+contents['data']
                            [i]['baseDate'][5:7]+contents['data'][i]['baseDate'][8:10])
                data.append(int(contents['data'][i]['baseDate'][0:4]+contents['data']
                            [i]['baseDate'][5:7]+contents['data'][i]['baseDate'][8:10]))
